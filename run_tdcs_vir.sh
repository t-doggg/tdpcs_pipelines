...
echo ""
echo "$(datetime.done) VIRUS-DEV Mode" 
...
#-------------------------------------------------------------------------------------------------------------------------------------
	...
	run_blastn() {
	    ...
	    #blastn -task megablast -db "$DATABASE_PATH" -num_threads "$THREADS" -outfmt '6 qseqid sacc stitle ssciname nident qlen' -max_target_seqs 1 -max_hsps 1 -query "$pre_filtered_fasta" > "$ncbi_unbinned_dir"
...
# Filter rows where the 5th column is >= 200 and save the output to a new file.
...
# Definiere den Pfad zur Protokolldatei für BLASTn
RESTRUC_LOG="$OUTDIR/03-Results/restruc.log"
